...
#'
#' \code{bpr_diff_classify_wrap} is a function that wraps all the necessary
...
#'
#' @export
bpr_diff_classify_wrap <- function(formula = NULL, x, y, model_name = "svm",
    ...
    # Convert from numerical to factors to perform classification
    ind <- which(y_diff > 0)
    y_diff[ind] <- 1
    y_diff[-ind] <- 0
    y_diff <- as.factor(y_diff)
    ...
    train_model <- train_classification_model(formula    = formula,
    ...
    predictions <- predict_classification_model(model      = train_model$gex_model,
                                                test       = dataset$test,
                                                is_summary = is_summary)
    message("Done!\n\n")
