#' Read Bismark Cov single cell formatted BS-Seq file
#'
#' \code{read_bs_sc} reads a file containing methylation data from BS-Seq
#' experiments using the \code{\link[data.table]{fread}} function. The BS-Seq
...
#'
#' @export
read_bs_sc <- function(file, chr_discarded = NULL, is_GRanges = TRUE){
    message("Reading file ", file, " ...")
    bs_data <- data.table::fread(input = file,
                                 sep = "\t",
                                 header = FALSE,
                                 col.names = c("chr", "start", "strand", "meth_reads",
                                               "unmeth_reads"))
