...
test_that("bpr_likelihood function works fine", {
  ...
  data <- cbind(obs, data)
  w <- c(.1, .1)
  expect_gt(bpr_likelihood(w, des_mat$H, data, 1/2, FALSE), -5.8)
  expect_lt(bpr_likelihood(w, des_mat$H, data, 1/2, FALSE), -5.7)
})
...
test_that("bpr_gradient function works fine", {
  ...
  data <- cbind(obs, data)
  w <- c(.1, .1, 1)
  expect_gt(bpr_gradient(w, des_mat$H, data, 1/2, FALSE)[1], -15.4)
  expect_lt(bpr_gradient(w, des_mat$H, data, 1/2, FALSE)[1], -15.2)
})
